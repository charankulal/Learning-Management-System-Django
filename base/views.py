...
def studentMyCourses(request, pk):
    student = Student.objects.get(name=pk)
    purchasedCourses = PurchaseAndEnrolment.objects.filter(
...
def studentMyCourseStudy(request, sk, pk):
    ...
    try:
        if request.session['email'] == student.email:
            ...
            context2 = {'teacher': teacher, 'student': student1}
            context3 = {'student': student,
                        'purchasedCourses': purchasedCourses, 'topics': topics}
            context = {**context1, **context2, **context3}
    except:
        return redirect('login')
...
def studentBuyCourse(request, sk, pk):
    ...
    try:
        if request.session['email'] == student.email:
            ...
            try:
                ispurchased = PurchaseAndEnrolment.objects.get(
                    course_id=course.id, student_id=student.id)
            except:
                ispurchased = None

            if ispurchased == None:
                new_purchase = PurchaseAndEnrolment(teacher_id=course.teacher_id, student_id=Student.objects.get(
                    id=student.id), course_id=Course.objects.get(id=course.id), amount=course.price)
                new_purchase.save()
...
def teacherRegister(request):
    if request.method == 'POST':
        ...
        new_Teacher = Teacher(name=name, email=email, password=password,
                              phone=phone, address=address, age=age, gender=gender)
        new_Teacher.save()
        ...
        if request.session['email'] == teacher.email:
            if request.GET.get('q') != None:
                ...
                course = Course.objects.filter(
                    teacher_id=teacher.id, courseName__icontains=q)
...
def studentsEnrolled(request, sk, pk):
    teacher = Teacher.objects.get(name=sk)
    course = Course.objects.get(courseName=pk)
    enrolls = PurchaseAndEnrolment.objects.filter(
        teacher_id=teacher.id, course_id=course.id)
    try:
        if request.session['email'] == teacher.email:
            context = {'teacher': teacher,
                       'course': course, 'enrolls': enrolls}
    except:
        return redirect('teacherloginpage')
    ...
    try:
        if request.session['email'] == teacher.email:
            ...
            if request.method == 'POST':
                ...
                s.starttls()
                s.set_debuglevel(1)
                msg = MIMEText("Added new course : "+courseName +
                ...
                new_course = Course(courseName=courseName, teacher_id=Teacher.objects.get(
                    name=pk), description=description, content=content, price=price, category=category)
                new_course.save()
        ...
        if request.session['email'] == teacher.email:
            if request.method == 'POST':
                ...
                new_course = Course.objects.get(
                    id=course.id, teacher_id=Teacher.objects.get(name=pk))
...
def addTopic(request, sk, pk):
    ...
    context = {'teacher': teacher, 'course': course}
    purchasedusers = PurchaseAndEnrolment.objects.filter(
        teacher_id=Teacher.objects.get(name=sk), course_id=Course.objects.get(courseName=pk))
    emails = [teacher.email]
    try:
        if request.session['email'] == teacher.email:
            for email in purchasedusers:
            ...
            if request.method == 'POST':
                content = request.POST['content']
                new_topic = Topic(teacher_id=Teacher.objects.get(
                    name=sk), course_id=Course.objects.get(courseName=pk), content=content)
                new_topic.save()
                s = smtplib.SMTP('smtp.gmail.com', 587)
                s.starttls()
                s.set_debuglevel(1)
